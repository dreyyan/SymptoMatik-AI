const Styles = {
  // Div
  mainDivStyle:
    "bg-[url(login-bg.png)] bg-cover bg-end flex flex-row w-full min-h-screen bg-white",
  loginDivStyle:
    "bg-gradient-to-l flex flex-col px-24 pt-2 pb-8 w-[40%] h-auto ml-14 mt-5",
  heroSectionDivStyle: "flex flex-col justify-center mb-[20%]",
  sidebarLinkDivStyle:
    "flex flex-col items-center justify-start items-end w-full h-full pr-[10%] text-2xl font-semibold border-b-2",
  signUpLinkDivStyle:
    "flex flex-row justify-center items-center gap-1 my-2 text-sm",
  ...
  // Components
  searchBarStyle:
    "border-2 border-[var(--trust-blue)] rounded-full w-[28%] h-[54%] pl-[1%] ml-8 mr-4",
  ...
  sidebarLinkStyle:
    "text-lg poppins-semibold relative text-[var(--trust-blue)]",
